...
def epfed_temporalattention(local_updates, global_model, args, momentum=None, global_masks=None):
    ###########################
    ########## local ##########
    ###########################
    if global_masks is not None:
        global_param_mask = global_masks[0]
        global_residual_mask = global_masks[1]
        global_mask = torch.logical_and(global_param_mask, global_residual_mask).float()
        print("comp rate: " + str(torch.sum(global_mask)/args.dim))
    ...
    for param in local_updates:
        ...
        if global_masks is None:
            global_param_mask = torch.zeros_like(flat_param)
            global_residual_mask = torch.zeros_like(flat_param)
            ...
            global_param_mask[indices] = 1
            global_residual_mask[indices] = 1
    ...
    # compute topk mask
    global_param_mask = model_topk(flat_global_model, args)
    global_residual_mask = model_topk(avg_local_update, args)

    return global_model, momentum, [global_param_mask, global_residual_mask]
