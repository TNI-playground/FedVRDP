...
def epfed_ablation_CR(local_updates, global_model, args, momentum=None, global_masks=None, best_mask=True):
    ###########################
    ########## local ##########
    ###########################
    flat_local_updates = []
    if best_mask:
        global_mask = torch.Tensor(np.loadtxt('best_mask.txt')).to(device=args.device)
    else:
        if global_masks is not None:
            global_mask_1 = global_masks[1]
            global_mask_0 = global_masks[0]
            global_mask = torch.mean(torch.cat([global_mask_1.unsqueeze(0), global_mask_0.unsqueeze(0)], dim=0), dim=0)
            comp_rate = torch.sum(torch.eq(global_mask, 0)) / args.dim
            print('comp_rate '+str(1-comp_rate.item()))
    ...
    for param in local_updates:
        ...
        if best_mask:
            ...
            if global_masks is None:
                global_mask_1 = torch.zeros_like(flat_param)
                global_mask_0 = torch.zeros_like(flat_param)
                random_mask = torch.randn_like(flat_param)
                _, indices = torch.topk(random_mask**2, int(args.com_p * args.dim))
                global_mask_1[indices] = 1
                global_mask_0[indices] = 1
                global_mask = torch.mean(torch.cat([global_mask_1.unsqueeze(0), global_mask_0.unsqueeze(0)], dim=0), dim=0)
    ...
    if best_mask:
        ...
        return global_model, momentum, [global_mask, global_mask_1]
