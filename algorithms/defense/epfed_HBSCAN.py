...
def epfed_HBSCAN(local_updates, global_model, args, momentum=None, global_mask=None):
    ###########################
    ########## local ##########
    ###########################
    flat_local_updates = []
    ...
    for i in range(args.num_selected_users):
        ...
        for j in range(args.num_selected_users):
            cos_ij = 1- cos(flat_local_updates[i],flat_local_updates[j])
            cos_i.append(cos_ij.item())
        cos_list.append(cos_i)
    clusterer = hdbscan.HDBSCAN(min_cluster_size=args.num_selected_users//2 + 1,
                                min_samples=1,
    ...
    if clusterer.labels_.max() < 0:
        for i in range(len(flat_local_updates)):
            benign_client.append(i)
            # norm_list = np.append(norm_list,
            #                       torch.norm(flat_local_updates[i], p=2).item())
    else:
        ...
        for i in range(len(clusterer.labels_)):
            if clusterer.labels_[i] == max_cluster_index:
                benign_client.append(i)
                # norm_list = np.append(norm_list,
                #                       torch.norm(flat_local_updates[i],p=2).item())  # no consider BN
    ...
    benign_flat_local_updates = []
    for i in range(len(benign_client)):
        benign_flat_local_updates.append(flat_local_updates[benign_client[i]])
    # clustering end

    # flatten models
    local_updates_vector = torch.stack(benign_flat_local_updates, dim=0)
