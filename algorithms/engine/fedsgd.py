...
def fedsgd(args):
    ...
    for t in range(args.round):
        ...
        for i in selected_idxs:
            samples, labels = next(iter(data_loader_list[i]))
            local_model, local_loss = local_solver.sgd(
                net=copy.deepcopy(net_glob).to(args.device),
                samples=samples.to(args.device), labels=labels.to(args.device))
        ...
        net_glob.load_state_dict(global_model)
        net_glob.eval()
        test_acc_t, test_loss = test_img(net_glob, dataset_test, args)
        
        # metrics
        test_acc.append(test_acc_t)
        print('t {:3d}: train_loss = {:.3f}, norm = {:.3f}, test_acc = {:.3f}'.
              format(t, train_loss[-1], median_model_norm[-1], test_acc[-1]))
        
        # stop
        if math.isnan(train_loss[-1]) or train_loss[-1] > 1e8 or t == args.round - 1:
