    ...
    def update_main_agent(self, round_no):
        ...
        with torch.no_grad():
            ...
            for i in range(number_of_samples):
                conv1_mean_weight += self.clients[self.client_names[i]].dqn.conv1.weight.clone()+ torch.normal(0, sigma, conv1_mean_weight.shape).to(device)
                conv1_mean_bias += self.clients[self.client_names[i]].dqn.conv1.bias.clone()+ torch.normal(0, sigma, conv1_mean_bias.shape).to(device)
                
                conv2_mean_weight += self.clients[self.client_names[i]].dqn.conv2.weight.clone()+ torch.normal(0, sigma, conv2_mean_weight.shape).to(device)
                conv2_mean_bias += self.clients[self.client_names[i]].dqn.conv2.bias.clone()+ torch.normal(0, sigma, conv2_mean_bias.shape).to(device)

                conv3_mean_weight += self.clients[self.client_names[i]].dqn.conv3.weight.clone()+ torch.normal(0, sigma, conv3_mean_weight.shape).to(device)
                conv3_mean_bias += self.clients[self.client_names[i]].dqn.conv3.bias.clone()+ torch.normal(0, sigma, conv3_mean_bias.shape).to(device)

                linear1_mean_weight += self.clients[self.client_names[i]].dqn.fc1.weight.clone()+ torch.normal(0, sigma, linear1_mean_weight.shape).to(device)
                linear1_mean_bias += self.clients[self.client_names[i]].dqn.fc1.bias.clone()+ torch.normal(0, sigma, linear1_mean_bias.shape).to(device)

                linear2_mean_weight += self.clients[self.client_names[i]].dqn.fc2.weight.clone()+ torch.normal(0, sigma, linear2_mean_weight.shape).to(device)
                linear2_mean_bias += self.clients[self.client_names[i]].dqn.fc2.bias.clone()+ torch.normal(0, sigma, linear2_mean_bias.shape).to(device)
